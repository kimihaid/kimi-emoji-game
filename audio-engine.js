...
class AudioEngine {
    ...
    generateNoise(duration, intensity = 0.1) {
        const bufferSize = Math.floor(duration * this.sampleRate);
        const buffer = this.audioContext.createBuffer(1, bufferSize, this.sampleRate);
        const data = buffer.getChannelData(0);
    ...
    // Apply envelope (ADSR)
    applyEnvelope(buffer, attack = 0.01, decay = 0.1, sustain = 0.7, release = 0.2) {
        const data = buffer.getChannelData(0);
    ...
    combineBuffers(buffers, gains = null) {
    ...
    generateSweep(startFreq, endFreq, duration, waveType = 'sine') {
        const bufferSize = Math.floor(duration * this.sampleRate);
        const buffer = this.audioContext.createBuffer(1, bufferSize, this.sampleRate);
        const data = buffer.getChannelData(0);
    ...
    generateBellSound() {
        ...
        return this.applyEnvelope(bell, 0.01, 0.3, 0.3, 1.2);
    }
    ...
    generateCarSound() {
        ...
        return this.applyEnvelope(car, 0.05, 0.1, 0.9, 0.65);
    }

    generateLizardSound() {
        // Squeaky lizard chirp: high pitched quick chirp
        const chirp = this.generateSweep(1200, 1800, 0.3, 'sine');
        return this.applyEnvelope(chirp, 0.01, 0.05, 0.6, 0.24);
    }
    ...
    generateBoomSound() {
        ...
        return this.applyEnvelope(boom, 0.01, 0.2, 0.3, 0.29);
    }
